<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('isi_rapats.index')" :active="request()->routeIs('isi_rapats.*')">
                        {{ __('Notula Rapat') }}
                    </x-nav-link>

                    @unless(auth()->user()->role === 'pimpinan')
                        <x-nav-link :href="route('notifs.index')" :active="request()->routeIs('notifs.*')">
                            {{ __('Notifikasi') }}
                        </x-nav-link>
                    @endunless

                    @unless(auth()->user()->role === 'user')
                        <x-nav-link :href="route('ruangans.index')" :active="request()->routeIs('ruangans.*')">
                            {{ __('Ruangan') }}
                        </x-nav-link>
                    @endunless
                </div>
    ...
    <div :class="{ 'block': open, 'hidden': ! open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('isi_rapats.index')" :active="request()->routeIs('isi_rapats.*')">
                {{ __('Notula Rapat') }}
            </x-responsive-nav-link>

            @unless(auth()->user()->role === 'pimpinan')
                <x-responsive-nav-link :href="route('notifs.index')" :active="request()->routeIs('notifs.*')">
                    {{ __('Notifikasi') }}
                </x-responsive-nav-link>
            @endunless
